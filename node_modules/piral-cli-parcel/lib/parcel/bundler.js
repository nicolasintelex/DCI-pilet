...
async function applySourceMapShift(sourceFile, lineOffset = 1) {
    ...
    const consumer = await new source_map_1.SourceMapConsumer(incomingSourceMap);
 ...
 */
async function postProcess(bundle, name, version, minified, importmap, isDependency = false) {
    const prName = `parcelChunkpr_${name.replace(/\W/gi, "")}`;
    const bundles = gatherJsBundles(bundle);
    const externals = gatherExternals(bundle);
    await Promise.all(bundles.map(async ({ src, css, map, parent }) => {
...
async function wrapJsBundle(details, content) {
 ...
 */
async function wrapUmd(details, content) {
...
function getRequireId(importmap, depName) {
    ...
    return dependency?.requireId ?? dependency?.id ?? depName;
}
/**
 * Transforms a pilet's bundle to a SystemJS entry module.
 */
async function wrapSystemJs(details, content) {
